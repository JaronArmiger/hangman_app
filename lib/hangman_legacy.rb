...
class Game
	attr_accessor :word, :guess, :display
	def initialize
		if load_game?
			load_game
			show_console
		else
			puts "Guess my word by guessing letters! If you get 6 letters wrong you lose!"
			@incorrect_guesses = 0
	...
	def update_display(word, indices)
		indices.each do |i|
			@display[i] = @word[i]
		end
	end
